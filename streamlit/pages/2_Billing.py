...
def create_checkout_session(req: CheckoutRequest) -> Dict[str, Any]:
    ...
    # Default return URLs
    # We intentionally send the user back to /Billing with enough context for the UI to:
    #   1) show a "success" confirmation
    #   2) persist the billing email
    #   3) clearly guide them to the Upload Data page
    #   4) optionally auto-navigate them to Upload Data
    #
    # NOTE: Stripe will also append `session_id={CHECKOUT_SESSION_ID}` to success_url.
    # We include email/plan so the Billing page can show the right messaging without
    # additional user input.
    from urllib.parse import quote

    success_url = os.getenv("SUCCESS_URL") or (
        f"{FRONTEND_URL}/Billing?status=success&email={quote(email)}&plan={quote(plan)}"
    )
    cancel_url = os.getenv("CANCEL_URL") or (
        f"{FRONTEND_URL}/Billing?status=cancelled&email={quote(email)}"
    )

    # Stripe requires success_url to contain the literal placeholder
    # `{CHECKOUT_SESSION_ID}`. Also, if success_url already has query
    # params (it does), we must append `&session_id=...` not `?session_id=...`.
